        package com.dozee.userdetails.services.Impl;
...
import com.dozee.userdetails.repository.DepartmentRepository;
...
public class UserServiceImpl implements UserService {
    ...
    private final transient DepartmentRepository departmentRepository;

    @Autowired
    private PasswordEncoder passwordEncoder;

    @Autowired
    public UserServiceImpl(final UserRepository userRepository, final DepartmentRepository departmentRepository) {
        super();
    ...
    public boolean saveUser(UserDetails userDetails) throws UserAlreadyExistsException {
        ...
        Optional<Department> dt = departmentRepository.findByNameAndRole(userDetails.getDepartment().getName(),
                userDetails.getDepartment().getRole());
        if (ud.isPresent()) {
        ...
        if(!dt.isPresent()){
            department1.setName(userDetails.getDepartment().getName());
            department1.setRole(userDetails.getDepartment().getRole());
            departmentRepository.save(department1);
        }
        dt = departmentRepository.findByNameAndRole(userDetails.getDepartment().getName(),
                userDetails.getDepartment().getRole());
        if(dt.isPresent()) {
            user.setDepartment(dt.get());
        }
